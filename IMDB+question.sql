...
INNER JOIN
	...
	USING(movie_id)
WHERE 
	(MONTHNAME(m.date_published) = 'MARCH' AND m.year = 2017) AND m.country LIKE '%USA%' AND r.total_votes > 1000
GROUP BY
	g.genre
ORDER BY
...
INNER JOIN 
	...
	ON m.id=r.movie_id
WHERE 
	r.avg_rating>8 AND	g.genre IN (SELECT genre FROM genre_top3)
GROUP BY 
	director_name
ORDER BY 
	movie_count DESC
...
INNER JOIN 
	...
	ON rm.name_id=nm.id
WHERE 
	category='actress' AND m.country LIKE '%India%' AND m.languages LIKE '%Hindi%'
GROUP BY 
	name
HAVING 
	COUNT(m.country='India')>=3
LIMIT 5;
...
SELECT
    ...
    DENSE_RANK() OVER(ORDER BY COUNT(r.movie_id) DESC) prod_company_rank
FROM
	movie m
INNER JOIN
	ratings r
	ON m.id = r.movie_id
WHERE 
	production_company IS NOT NULL AND (r.median_rating >=8) AND (position(',' IN m.languages)>0)
GROUP BY
	m.production_company
LIMIT 2;
...
--------------------------------------------------------------------------------------------*/
-- Type you code below:
WITH cte_required_columns AS(
	SELECT 
		dm.name_id,
		n.name,
		m.id,
		m.date_published,
		LEAD(m.date_published, 1) OVER(PARTITION BY dm.name_id ORDER BY m.date_published, m.id) AS next_movie_date,
		r.avg_rating,
		r.total_votes,
		m.duration
	FROM
